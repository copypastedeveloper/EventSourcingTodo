...
namespace Example.Data.MongoDb.UnitTests
{
    public class mongo_repository_finds_item_to_remove : Scenario
    {
        ...
        given a_mongo_repository = () =>
        {
            ...
            _mockMongoCollection.Setup(x => x.Remove(It.IsAny<QueryDocument>()))
                .Returns(new WriteConcernResult(new BsonDocument(new Dictionary<string, object> {{"n", 1}})));
        ...
        [then]
        public void no_delete_should_be_logged()
        {
            _mockLog.VerifyAll();
        }
    }
